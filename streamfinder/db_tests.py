...
class FinderDbTestCase(AbstractFinderDbTestCase):
	...
	def test_displayed_match_json(self):
		# Create the second team.
		team2_id = 'team2_id_value'
		team2_name = 'team2_name_value'
		team2_num_stars = 2
		team2 = db.DisplayedTeam(team2_id, team2_name, team2_num_stars)
		# Create the match without optional values.
		match_id = 'match_id_value'
		match_time = datetime(2012, 5, 5, 12, 30, 0)
		match_num_stars = 3
		match_num_streams = 4
		match = db.DisplayedMatch(match_id, None, team2,
				match_time, match_num_stars, match_num_streams)
		...
		expected_json = {
			...
			'num_stars': match_num_stars,
			'num_streams': match_num_streams,
		}
		self.assertDictEqual(expected_json, match.to_json())

		# Create the first team.
		team1_id = 'team1_id_value'
		team1_name = 'team1_name_value'
		team1_num_stars = 1
		team1 = db.DisplayedTeam(team1_id, team1_name, team1_num_stars)
		# Create the match with optional values.
		match_game = 'match_game_value'
		match_league = 'match_league_value'
		match = db.DisplayedMatch(match_id, team1, team2,
				match_time, match_num_stars, match_num_streams,
				game=match_game, league=match_league)

		expected_json['team1'] = team1.to_json()
		expected_json['game'] = match_game
		expected_json['league'] = match_league
		self.assertDictEqual(expected_json, match.to_json())
	
	def test_displayed_team_json(self):
		# Create the team without optional values.
		team_id = 'team_id_value'
		team_name = 'team_name_value'
		team_num_stars = 1
		team = db.DisplayedTeam(team_id, team_name, team_num_stars)

		expected_json = {
			'team_id': team_id,
			'name': team_name,
			'num_stars': team_num_stars,
		}
		self.assertDictEqual(expected_json, team.to_json())

		# Create the team with optional values.
		team_game = 'team_game_value'
		team_league = 'team_league_value'
		team = db.DisplayedTeam(team_id, team_name, team_num_stars,
				team_game, team_league)

		expected_json['game'] = team_game
		expected_json['league'] = team_league
		self.assertDictEqual(expected_json, team.to_json())

	def test_displayed_streamer_json(self):
		# Create the streamer without optional values.
		streamer_id = 'streamer_id_value'
		streamer_name = 'streamer_name_value'
		streamer_num_stars = 1,
		streamer_image_url = 'streamer_image_url'
		streamer = db.DisplayedStreamer(
				streamer_id, streamer_name, streamer_num_stars, streamer_image_url)

		expected_json = {
			'streamer_id': streamer_id,
			'name': streamer_name,
			'num_stars': streamer_num_stars,
			'image_url': streamer.image_url,
		}
		self.assertDictEqual(expected_json, streamer.to_json())

		# Create the streamer with optional values.
		streamer_url_by_id = 'url_by_id_value'
		streamer_url_by_name = 'url_by_name_value'
		streamer = db.DisplayedStreamer(
				streamer_id, streamer_name, streamer_num_stars, streamer_image_url,
				streamer_url_by_id, streamer_url_by_name)

		expected_json['url_by_id'] = streamer_url_by_id
		expected_json['url_by_name'] = streamer_url_by_name
		self.assertDictEqual(expected_json, streamer.to_json())
